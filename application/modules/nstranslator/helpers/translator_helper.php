...
class Translate {
    ...
    public static function sprint($msgId="",$default=""){
        ...
        if(isset($data[$msgId])){
            ...
            */
            self::addNewWordIfNeeded($msgId,$default);
            //END
    ...
    private static function addNewWordIfNeeded($key,$value){
        ...
        //register new words to all languages
        $data = self::$_translate_data_array;
    ...
    public static function loadLanguageFromCache($def=""){
        ...
        if($def!="" && preg_match("#[a-zA-Z]{2}#",$def)){
            $fileJSON = Path::getPath(array("uploads","translates",trim($def).".json"));
        }else if($lngFromSession!=""){
            $fileJSON = Path::getPath(array("uploads","translates",trim($lngFromSession).".json"));
        }else{
    ...
    private static function loadJSONs(){
