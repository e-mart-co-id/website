...
class Pack_model extends CI_Model
{
    ...
    public function confirmPayment($args = array())
    {
        ...
        if (isset($args['invoiceId'])) {
            $invoiceId = intval($args['invoiceId']);
        } else
            $invoiceId = 0;
    ...
    public function sendRemind($user_id = 0, $pack_id = 0)
    {
        ...
        if ($user != NULL && $pack != NULL) {
            ...
            $mail = new Mailer();
            $mail->setDistination($user['email']);
            ...
            $mail->setType("html");
            $mail->setSubjet(Translate::sprintf('Your pack %s will expired soon', array($pack->name)));
            $mail->send();
    ...
    public function sendPackConfirmation($user_id, $pack_id)
    {
        ...
        if ($user != NULL && $pack != NULL && $invoice != NULL) {
            ...
            $path = FCPATH . '/application/modules/pack/views/mailing/' . DEFAULT_LANG . '_pack_confirmation.php';
            if (file_exists($path)) {
                $html = $this->load->view('pack/mailing/' . DEFAULT_LANG . '_pack_confirmation', NULL, TRUE);
            } else {
                $html = $this->load->view('pack/mailing/en_pack_confirmation', NULL, TRUE);
            }
            ...
            $mail->setReplay_to_name(APP_NAME);
            $mail->setType("html");
            $mail->setSubjet(Translate::sprint(APP_NAME . " - Your pack \"$pack->name\" is activated"));
            $mail->send();
