...
class Payment_model extends CI_Model
{
    ...
    public function updateInvoice($id,$method,$tranid,$key='',$redirect=TRUE){
        ...
        if(isset($invoice[0])){
            ...
            if($result==TRUE and $key==md5("abc-key".$id)){
                ...
                if($redirect)
                    redirect(PaymentsProvider::getErrorCallback($invoice->module));
                else
                    return TRUE;
            }
    ...
    public function getInvoices($params =array()){
        ...
        $this->db->group_by("invoice.created_at","DESC");

        if(isset($order_by_date) and $order_by_date==1)
            $this->db->order_by("invoice.created_at","DESC");
        else
            $this->db->order_by("invoice.created_at","ASC");
