...
//                $this->db->update("user");
        ...
        if (empty($errors)) {
            ...
            if ($user->num_rows() == 1) {
                ...
                if (isset($guest_id) and $guest_id > 0) {
                    ...
                    //remove old guest
                    if ($user_data[0]['guest_id'] > 0 && $user_data[0]['guest_id'] != $guest_id) {
                        $this->db->where('id', $user_data[0]['guest_id']);
    ...
    public function signUp($params = array(), $fieldsRequirement = array())
    {
        ...
        if (empty($errors)) {
            ...
            if ($count == 1) {
                $errors['user'] = Translate::sprint("User is not exists");
            }
        }
        ...
        if (empty($errors) and !empty($data)) {
            ...
            if (isset($guest_id) and $guest_id > 0) {
                ...
                //remove the guest id from old users
                $this->db->where("guest_id", intval($guest_id));
                $this->db->update("user", array(
                    "guest_id" => 0
                ));

                //affacted to the other user
                $this->db->where("id_user", $user_data[0]['id_user']);
                $this->db->update("user", array(
                    "guest_id" => intval($guest_id)
                ));
    ...
    public function getUsersAnalytics($months = array(), $owner_id = 0)
    {
        ...
        $analytics['count_label'] = Translate::sprint("Total_users");
